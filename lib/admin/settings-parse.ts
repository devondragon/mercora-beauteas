 ...
 *
 *   currency = USD
 *   promo_message = 🍵 Holiday Sale: 30% off all teas through...
 *   social_instagram = https://instagram.com/beauteas
 *   store_name = BeauTeas          (+ promo_link, promo_link_text, store_tagline,
 *                                   store_description, social_facebook,
 *                                   social_linkedin, social_pinterest)
 *
 * The admin settings page used to call `JSON.parse(setting.value)` unguarded
 * inside a `forEach`. The first legacy row threw, the loop aborted, the outer
 ...
 * state, one click of Save wrote defaults over all 31 real settings: it turned
 * per-box shipping off, restored free shipping over $75, re-enabled the express
 * and overnight methods that migration 0028 disabled, and replaced the closing
 * sale's banner copy. Nothing errored, and the page looked normal throughout,
 * because the defaults are plausible values.
 *
...
export function parseSettingValue(raw: unknown): unknown {
  ...
  try {
    ...
    // A bare legacy string ("USD", "https://instagram.com/beauteas"). Returning
    // it as-is is what the row means; the alternative that shipped was losing
    // every other row alongside it.
    return raw;
  }
}
