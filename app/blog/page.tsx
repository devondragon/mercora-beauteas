...
export default async function BlogIndexPage() {
  ...
  return (
    <div className="min-h-screen">
      {/* Header */}
      <section className="bg-gradient-to-b from-primary-100 to-surface-dark py-16 text-center">
        <h1 className="text-4xl font-bold tracking-tight text-text-primary md:text-5xl">Beauty Journal</h1>
      ...
      <div className="mx-auto max-w-6xl px-4 pb-16 sm:px-6 lg:px-8">
        ...
        {posts.length === 0 ? (
          <p className="py-20 text-center text-text-muted">No posts yet. Check back soon!</p>
        ) : (
          <div className="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3" id="posts-grid">
            {posts.map((post) => (
              <article key={post.slug} data-post-tags={JSON.stringify(post.tags)}>
                <Link
                  href={`/blog/${post.slug}`}
                  className="group flex h-full flex-col overflow-hidden rounded-xl border border-border-default bg-white transition hover:-translate-y-0.5 hover:border-secondary-400 hover:shadow-lg"
                >
                  ...
                  <div className="flex flex-1 flex-col p-6">
                    ...
                    <h2 className="mb-2 flex-1 text-lg font-semibold text-text-primary group-hover:text-primary-600">
                      {post.title}
                    </h2>
                    {post.excerpt && (
                      <p className="mb-4 line-clamp-3 text-sm text-text-secondary">{post.excerpt}</p>
                    )}
