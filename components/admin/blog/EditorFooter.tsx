...
function Kbd({ children }: { children: React.ReactNode }) {
  return (
    <kbd className="rounded border border-border-default bg-surface px-1.5 py-0.5 font-mono text-[10px] text-secondary-600">
      {children}
    </kbd>
  );
}
...
export function EditorFooter({ editor }: { editor: Editor | null }) {
  ...
  return (
    <div className="flex items-center justify-between border-t border-border-default bg-white px-4 py-2 text-xs text-text-muted">
      <div className="flex items-center gap-3 flex-wrap">
        <span><Kbd>/</Kbd> commands</span>
        <span><Kbd>Tab</Kbd><Kbd>Tab</Kbd> <Sparkles className="inline h-3 w-3 text-secondary-600" /> AI suggest</span>
